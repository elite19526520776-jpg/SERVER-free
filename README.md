...
启动后会打印 REST / WebSocket 地址，以及**本机的局域网 IP**——手机真机调试时要用这个地址，不能用 localhost。
...
15 个端到端用例，会真的起一个 HTTP + WebSocket 服务，用共享层的客户端跑完整流程：
注册登录、鉴权拒绝、搜人、建会话去重、实时收发、ack、未读数、已读回执、
消息幂等、离线补发、历史翻页、越权访问拦截、伪造 token 拦截、正在输入转发。
